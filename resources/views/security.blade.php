...
<div>
    ...
    @else
        @php
            $dangers = count(array_filter($securityIssues, fn($i) => $i['type'] === 'danger'));
            $warnings = count(array_filter($securityIssues, fn($i) => $i['type'] === 'warning'));
            $infos = count(array_filter($securityIssues, fn($i) => $i['type'] === 'info'));
        @endphp
        ...
        <div class="bg-drac-surface rounded-xl border border-drac-border overflow-hidden mb-6">
            <div class="grid grid-cols-3 divide-x divide-drac-border">
                <div class="px-4 py-3.5">
                    <div class="text-drac-comment text-[10px] uppercase tracking-wider font-semibold mb-1">Critical</div>
                    <div class="text-lg font-extrabold text-drac-red leading-none">{{ $dangers }}</div>
                </div>
                <div class="px-4 py-3.5">
                    <div class="text-drac-comment text-[10px] uppercase tracking-wider font-semibold mb-1">Warnings</div>
                    <div class="text-lg font-extrabold text-drac-orange leading-none">{{ $warnings }}</div>
                </div>
                <div class="px-4 py-3.5">
                    <div class="text-drac-comment text-[10px] uppercase tracking-wider font-semibold mb-1">Info</div>
                    <div class="text-lg font-extrabold text-drac-cyan leading-none">{{ $infos }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-drac-surface rounded-xl border border-drac-border overflow-hidden">
            ...
            <div class="divide-y divide-drac-border/60">
                ...
                <div class="px-5 py-3.5 flex items-start gap-3.5 hover:bg-drac-current/30 transition">
                    ...
                    <div class="w-7 h-7 rounded-lg bg-drac-red/10 flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div class="w-7 h-7 rounded-lg bg-drac-orange/10 flex items-center justify-center shrink-0 mt-0.5">
                        <svg class="w-3.5 h-3.5 text-drac-orange" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>
                    </div>
                    @else
                    <div class="w-7 h-7 rounded-lg bg-drac-cyan/10 flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full uppercase
                                {{ $issue['type'] === 'danger' ? 'bg-drac-red/10 text-drac-red' : '' }}
                                {{ $issue['type'] === 'warning' ? 'bg-drac-orange/10 text-drac-orange' : '' }}
                                {{ $issue['type'] === 'info' ? 'bg-drac-cyan/10 text-drac-cyan' : '' }}
                            ">{{ $issue['category'] }}</span>
                        </div>
