...
class ModelCollector
{
    ...
    {
        Event::listen('eloquent.retrieved:*', function (string $event) {
            $this->record($event, 'retrieved');
        });

        Event::listen('eloquent.created:*', function (string $event) {
            $this->record($event, 'created');
        });

        Event::listen('eloquent.updated:*', function (string $event) {
            $this->record($event, 'updated');
        });

        Event::listen('eloquent.deleted:*', function (string $event) {
